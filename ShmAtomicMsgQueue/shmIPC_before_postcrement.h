 ...
 ***********************************************************************************/
    ...
    uint32_t read(void *pBuff, uint32_t buffSize, int32_t msTimeout = INT32_MAX) {
        ...
        // Copy the data //TODO: add more complicated read here to get a size (like socket lib does it)
...
//        printf("pBlock->Amount: %d\n", dwAmount);
    ...
    typename ::block<BLOCK_SIZE>* get_block_write(int32_t msTimeout = INT32_MAX) {
        ...
        while(1) {
            ...
            if (pBlock->Next == m_pBuffer->m_ReadEnd) { //todo
//            if (blockIndex == m_pBuffer->m_ReadEnd) { //todo

                //Setup timeout value
                boost::posix_time::ptime timeout =
                        microsec_clock::local_time() + boost::posix_time::milliseconds(msTimeout); //todo: remove with new events

                // No room is available, wait for room to become available
//                if (m_pBuffer->avail_sem.timed_wait(timeout))

                //printf("(GETBLKWRITE)R-S: %ld, R-E: %ld, W-S: %ld, W-E: %ld\n", (long)m_pBuffer->m_ReadStart, (long)m_pBuffer->m_ReadEnd, (long)m_pBuffer->m_WriteStart, (long)m_pBuffer->m_WriteEnd);
                if(WaitForEvent(&m_pBuffer->m_avail, (uint32_t)msTimeout))
                    continue;
    ...
    void post_block(Block *pBlock) {
        ...
        while(1) {
            ...
            if (pBlock->Prev == m_pBuffer->m_ReadStart) {
                //printf("Signalling that there is data to read\n");
...
//            if (pBlock->Next == m_pBuffer->m_WriteEnd) {
            if (blockIndex == m_pBuffer->m_WriteEnd) {
                ...
                //printf("(GETBLKREAD)R-S: %ld, R-E: %ld, W-S: %ld, W-E: %ld\n", (long)m_pBuffer->m_ReadStart, (long)m_pBuffer->m_ReadEnd, (long)m_pBuffer->m_WriteStart, (long)m_pBuffer->m_WriteEnd);
                // No room is available, wait for room to become available
//                if (m_pBuffer->signal_sem.timed_wait(timeout))
                if(WaitForEvent(&m_pBuffer->m_signal, (uint32_t)msTimeout))
                    continue;
    ...
    void return_block(Block *pBlock) {
        ...
        while(1) {
            ...
            if (pBlock->Prev == m_pBuffer->m_WriteStart) {
                //printf("Signalling that there is available space to write\n");
